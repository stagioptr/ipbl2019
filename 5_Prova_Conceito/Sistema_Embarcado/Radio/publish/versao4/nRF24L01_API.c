...
NRF24L01_state_t nRF24L01_Init(void) {
	...
	nRF24L01_writeRegister(CONFIG, 0x00); // Disable device

	nRF24L01_writeRegister(EN_AA, 0x01);            // Enable Auto.Ack:Pipe0
	nRF24L01_writeRegister(EN_RXADDR, 0x01);        // Enable pipe 0.
	nRF24L01_writeRegister(SETUP_RETR, 0x1a);  // 500탎 + 86탎, 10 retrans...
...
NRF24L01_state_t nRF24L01_Denit(void) {
	nRF24L01_writeRegister(CONFIG, 0x00);

	nRF24L01_writeRegister(FLUSH_RX, 0);
	nRF24L01_writeRegister(FLUSH_TX, 0);
	nRF24L01_writeRegister(STATUS, 0x70);

	nRF24L01_writeRegister(EN_AA, 0x00);
	nRF24L01_writeRegister(EN_RXADDR, 0x00);
	nRF24L01_writeRegister(SETUP_RETR, 0x00);
	nRF24L01_writeRegister(RF_CH, 0);
	nRF24L01_writeRegister(RF_SETUP, 0x00);
